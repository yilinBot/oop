...
//Date:     2017.12.15.
//Title:    The most frequent number - type-oriented version
...
//           and gives the most frequent number of this bag.
int main()
{
...
TEST_CASE("putting an illegal element", "[putIn]")
{
    ...
    vector<int> v = { 1, 2 };
    int e = b.mostFrequent();
    CHECK(v == b.getArray());
    CHECK_THROWS(b.putIn(2));
    CHECK(v == b.getArray());
    CHECK(e == b.mostFrequent());
}

TEST_CASE("empty bag", "[mostFrequent]")
{
    int m = 1;
    Bag_Test b(m);
    CHECK_THROWS(b.mostFrequent());
...
TEST_CASE("one element in the bag", "[mostFrequent]")
{
    int m = 1;
    Bag_Test b(m); b.putIn(0);
    vector<int> v = { 1, 0 };
...
TEST_CASE("one most frequent element", "[mostFrequent]")
{
    int m = 1;
...
TEST_CASE("more most frequent elements", "[mostFrequent]")
{
    int m = 1;
    Bag_Test b(m); b.putIn(0); b.putIn(0); b.putIn(1); b.putIn(1);
    vector<int> v = { 2, 2 };
    CHECK(v == b.getArray());
    bool l = (0 == b.mostFrequent()) || (1 == b.mostFrequent());
    CHECK(l);
}

TEST_CASE("most frequent element : 0", "[integration]")
{
    int m = 1;
    Bag_Test b(m); b.putIn(0);
    vector<int> v = { 1, 0 };
...
TEST_CASE("most frequent element : m", "[integration]")
{
    int m = 1;
